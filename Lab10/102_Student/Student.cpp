...
//}
Student::Student(std::string name, std::string last_name, int id, std::string cat) {
    Student::set_name(name);
    Student::set_last_name(last_name);
    Student::set_id_card(id, cat);
}
...
//}
void Student::set_id_card(int id, std::string c) {
    iCard = IdCard(id, c); // create inner object
}
int Student::get_id_card() const {
    return iCard.get_number();
}

// Student info
void Student::display() const {
    std::cout 
        << "Name | " << "LastName | " << "AvgScore | " << "Id | " << "Type\n"
        << last_name << ' ' << name << "\t"
        << average_score << "\t"
        << iCard.get_number() << "\t"
        << iCard.get_category() << std::endl;
}
